...
#            [QgsProcessing.TypeVectorPoint]))
        ...
        self.addParameter(QgsProcessingParameterFileDestination(self.OUTPUT, 'XYZ grid', '*.csv', defaultValue=None))
    ...
    def importingandcounting(self,parameters):
        ...
        for i in range(count):
            ...
            if self.XYZ[i,1]<self.ymax and self.XYZ[i,1]>self.ymin and self.XYZ[i,0]<self.xmax and self.XYZ[i,0]>self.xmin:
                values[NumPxl[i,1].astype(int),NumPxl[i,0].astype(int)]=self.XYZ[i,2]
        #print(values)
